...
/*
	...
	Ellipse function is missing from the GDI bindings for odin so creating it instead
*/
...
foreign gdi32 {
	Ellipse :: proc(hdc: win.HDC, left: win.INT, top:win.INT, right:win.INT, bottom:win.INT) -> win.BOOL ---
}
...
window_event_proc :: proc "stdcall" (
	...
	switch message {
		...
		case win.WM_PAINT:
			...
			Ellipse(hdc = hdc, left = 50, top = 50, right = 200, bottom = 150)
			Ellipse(hdc = hdc, left = 100, top = 300, right = 200, bottom = 200)
...
main :: proc() {
	...
	window_class := win.WNDCLASSW {
		...
		hInstance = instance,
		lpszClassName = win.L("D3D11WindowClass"),		
	}
